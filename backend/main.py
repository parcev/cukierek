...
DISCORD_ACTIVITY_WEBHOOK_URL = os.getenv("DISCORD_ACTIVITY_WEBHOOK_URL")
...
app = FastAPI(title="Candy Dispenser API")
...
@app.middleware("http")
async def activity_logging_middleware(request, call_next):
    session_id = None
    platform = None
...
class ExtendSessionRequest(BaseModel):
    admin_key: str
    session_id: str
    extend_seconds: int = 240  # Default to 4 minutes
...
def is_valid_platform(platform):
    return platform in ["facebook", "instagram"]
...
def get_facebook_follower_count():
    ...
    url = f"https://graph.facebook.com/{META_API_VERSION}/{FB_PAGE_ID}"

    params = {"fields": "followers_count", "access_token": FB_ACCESS_TOKEN}
    ...
    if not response.ok:
        ...
        raise RuntimeError(f"Facebook Meta API error: {error_data}")
    ...
    if count is None:
        raise RuntimeError("Facebook Meta API did not return followers_count")
...
def get_instagram_follower_count():
    ...
    url = f"https://graph.facebook.com/{META_API_VERSION}/{IG_USER_ID}"
    ...
    if not response.ok:
        ...
        raise RuntimeError(f"Instagram Meta API error: {error_data}")
    ...
    if count is None:
        raise RuntimeError("Instagram Meta API did not return followers_count")
...
def send_to_adafruit(dispense_id):
    ...
    url = f"https://io.adafruit.com/api/v2/{ADAFRUIT_USERNAME}/feeds/cukierki/data"

    headers = {"X-AIO-Key": ADAFRUIT_KEY, "Content-Type": "application/json"}

    payload = {"value": f"DROP:{dispense_id}"}
...
def send_discord(dispense_id, platform=None, manual=False):
...
def send_discord_activity(
    method, path, status_code, session_id=None, platform=None, details=None
):
    ...
    try:
        requests.post(DISCORD_ACTIVITY_WEBHOOK_URL, json=payload, timeout=3)
...
def start_session(req: StartSessionRequest):
    ...
    with state_lock:
        ...
        try:
            facebook_initial = get_facebook_follower_count()

        except Exception as error:
            raise HTTPException(
                status_code=502, detail=(f"Nie można pobrać stanu Facebook: {error}")
            )

        # Pobierz początkową liczbę Instagram

        try:
            instagram_initial = get_instagram_follower_count()

        except Exception as error:
            raise HTTPException(
                status_code=502, detail=(f"Nie można pobrać stanu Instagram: {error}")
            )
        ...
        sessions[session_id] = {
            ...
            "facebook_started": req.platform == "facebook",
            "instagram_started": req.platform == "instagram",
            # Facebook
            "facebook_initial": facebook_initial,
            "facebook_dispensed": False,
            # Instagram
            "instagram_initial": instagram_initial,
            "instagram_dispensed": False,
        }

    return {"session_id": session_id, "expires_in": SESSION_DURATION}
...
def start_platform(req: StartPlatformRequest):
    if not is_valid_platform(req.platform):
        raise HTTPException(status_code=400, detail="INVALID_PLATFORM")
    ...
    with state_lock:
        ...
        started_key = f"{req.platform}_started"

        dispensed_key = f"{req.platform}_dispensed"
        ...
        return {
            ...
            "facebook_started": session["facebook_started"],
            "instagram_started": session["instagram_started"],
            "facebook_dispensed": session["facebook_dispensed"],
            "instagram_dispensed": session["instagram_dispensed"],
        }
...
def verify(req: VerifyRequest):
    ...
    with state_lock:
        ...
        initial_key = f"{req.platform}_initial"

        # Platforma musi być wcześniej rozpoczęta

        if not session[started_key]:
            raise HTTPException(status_code=409, detail="PLATFORM_NOT_STARTED")
        ...
        initial_count = session[initial_key]
    ...
    for attempt in range(FOLLOWER_CHECKS):
        try:
            current_count = get_follower_count(req.platform)

        except Exception as error:
            raise HTTPException(
                status_code=502, detail=(f"Błąd sprawdzania {req.platform}: {error}")
            )
    ...
    with state_lock:
        if now - last_dispense_time < DISPENSE_COOLDOWN:
            raise HTTPException(status_code=429, detail="TRY_AGAIN")
        ...
        session[f"{req.platform}_dispense_id"] = dispense_id
    ...
    try:
        ...
        with state_lock:
            ...
            if session:
                session[f"{req.platform}_dispensed"] = False

                session.pop(f"{req.platform}_dispense_id", None)

        raise HTTPException(status_code=502, detail=(f"Błąd Adafruit IO: {error}"))

    # Discord

    send_discord(dispense_id, platform=req.platform)
    ...
    with state_lock:
        ...
        if session:
            both_done = session["facebook_dispensed"] and session["instagram_dispensed"]
...
def manual_dispense(req: ManualDispenseRequest):
    ...
    dispense_id = "MANUAL_" + secrets.token_urlsafe(16)
...
# EXTEND / REACTIVATE SESSION


@app.post("/api/admin/extend-session")
def extend_session(req: ExtendSessionRequest):
    if req.admin_key != ADMIN_KEY:
        raise HTTPException(status_code=401, detail="UNAUTHORIZED")
    ...
    with state_lock:
        ...
        if session:
            # Extend existing active or unpurged session
            base_time = max(now, session["expires_at"])
            session["expires_at"] = base_time + req.extend_seconds
            status_msg = "Sesja została pomyślnie przedłużona."
        else:
            # Reactivate purged session by taking fresh follower counts
            try:
                facebook_initial = get_facebook_follower_count()
                instagram_initial = get_instagram_follower_count()
            except Exception as error:
                raise HTTPException(
                    status_code=502,
                    detail=f"Nie można pobrać stanu followerów: {error}",
                )

            sessions[req.session_id] = {
                "created_at": now,
                "expires_at": now + req.extend_seconds,
            ...
            status_msg = "Sesja została zreaktywowana."

        return {
            "status": "success",
            "message": status_msg,
            "session_id": req.session_id,
            "expires_at": sessions[req.session_id]["expires_at"],
            "remaining_seconds": int(sessions[req.session_id]["expires_at"] - now),
        }
